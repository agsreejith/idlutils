; $Id: //depot/idl/IDL_70/idldir/lib/bar_plot.pro#1 $
;
; Copyright (c) 1990-2007, ITT Visual Information Solutions. All
...
;
;	SUPRESSX: IDL's stupid procedure automatically doesn't label
;		the x-axis! WTF! Here's a keyword to supress that nasty
;		behavior. 
;
; OUTPUTS:
...
;-
pro plothistogram,values,baselines=baselines,colors=colors,barnames=barnamesIn, $
          ...
          rotate=rotate, supressx=supressx, yrng=yrng
...
nelb=n_elements(barnames)
if nelb gt 60 then begin
barnames=barnames[floor(nelb*dindgen(10)/9d)]
endif

if keyword_set(yrng) then yrange=yrng
...
if (rotate) then begin				   ;Label the bars (Y-axis)
  ...
  help, tickv
  if nelb gt 60 then begin
	tickv=tickv[floor(nelb*dindgen(10)/9d)]
	xticknum=10
  endif else xticknum= (nbars-1)
  help, tickv
  axis,xaxis=0,xstyle=1,xticks=xticknum,xtickv=tickv,xtickname=barnames, $
    xticklen=0.0
endelse
