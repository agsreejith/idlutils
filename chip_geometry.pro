...
function chip_geometry, fileName, hdr=hdr
	...
	namps = n_elements(strsplit(sxpar(hdr, 'amplist'), /extract))
	...
	if ( results.controller eq 'torrent' ) then begin
        ;from Tokovinin 20120301
      if strt(results.readout_speed) eq 'normal' then begin
		  results.gain.upleft = 1.30d
		  results.gain.upright = 1.25d
		  results.gain.botleft = 1.264d
		  results.gain.botright = 1.257d
		endif else begin
		  results.gain.upleft = 5.00d
		  results.gain.upright = 5.17d
		  results.gain.botleft = 4.73d
		  results.gain.botright = 5.40d
		endelse
	endif
