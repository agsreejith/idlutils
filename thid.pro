...
;Display threshold handler.
    if cmd eq 't' then begin
      read, 'Display threshold is currently ' + fmtnum(dmax, '(f10.2)') $
...
;Line location window handler.
    if cmd eq 'w' then begin
      read, ' line window is currently +/-' + fmtnum(mwin, '(f10.2)') $
...
;          wait, 0.2
          ...
          if bad eq 0 then begin
            ...
            print, 'pixel=' + fmtnum(ipix2, '(f10.2)')
          endif
